  ...
  <header id="header" class="fixed-top header-inner-pages">
    ...
    <section class="breadcrumbs">
      <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center">
        	<c:if test="${not empty requestScope.currentBusiness}">
          <h2>${requestScope.currentBusiness.name}</h2>
          </c:if>
          <c:if test="${empty sessionScope.currentBusiness}">
          <h2>Display Business</h2>
          </c:if>
          <ol>
            <li><a href="home">Home</a></li>
            <li>Display Business</li>
          </ol>
        </div>
    ...
    <section class="inner-page">
      <div class="container">
        <p>
		<c:if test="${empty sessionScope.currentBusiness}">
		<p><a href="addBusiness">Business not found, click here to add business</a></p>
		</c:if>
		<c:if test="${not empty sessionScope.currentBusiness}">
	<h5>Reviews</h5>
	<table class="table">
		<tr>
			<th scope="col">Id</th>
			<th scope="col">Accom Cost</th>
		...
		<c:forEach items="${requestScope.allBusinessReviews}" var="currentReview">
			<tr>
				<td>${currentReview.id}</td>
				<td>$${currentReview.accommodationCost}</td>
				<td>${currentReview.accommodationType}</td>
				<td>${currentReview.comment}</td>
				<td>$${currentReview.payRate}</td>
				<td>${currentReview.job.title}</td>
				<td>${currentReview.user.username}</td>
			</tr>
		</c:forEach>
	</table>
	<p><a href="findJobByTitle">Leave a review for this business</a></p>
        </p>
      </div>
    </c:if>
    </section>
